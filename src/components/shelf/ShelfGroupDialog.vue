...
<script>
import EbookDialog from '../Dialog'
import { storeShelfMixin } from '../../utils/mixin'
import { removeAddFromShelf, appendAddToShelf, addShelfData } from '../../utils/book'
...
export default {
  ...
  methods: {
    ...
    moveOutFromGroup(item) {
      let title = this.shelfCategory.title
      let newShelfCategory = this.shelfCategory.itemList.filter((item) => {
        return !this.shelfSelected.some(book =>
          book.bookId === item.bookId)
      })
      this.shelfCategory.itemList = [...newShelfCategory]
      this.setShelfCategory(this.shelfCategory)
      let shelfList = [...this.shelfList]
      shelfList.map((item) => {
        if (item.type === 2 && item.title === title) {
          item.itemList = [...newShelfCategory]
        }
      })
      this.shelfSelected.forEach((item) => {
        shelfList.unshift(item)
      })

      if (shelfList[shelfList.length - 1].type !== -1) {
        this.setShelfList(addShelfData(shelfList))
      } else {
    ...
    createNewGroup() {
      ...
      if (this.showNewGroup) {
        ...
        group.itemList.push(...this.shelfSelected)
        let list = removeAddFromShelf(this.shelfList)
        if (list[list.length - 1].type === -1) {
          list.pop()
        }
        list.push(group)
        list = appendAddToShelf(list)
        list = list.filter(item => {
          return !this.shelfSelected.some(book => book.bookId === item.bookId)
        })
